...
from timesformer.models.softdtw_loss import SoftDTW  # 모듈 임포트
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, preds, labels):
        # 기본 CrossEntropy Loss 계산
        loss = self.ce_loss(preds, labels)
        
        # SoftDTW Loss 추가 계산 (가중치 0.1 적용)
        if self.sdtw is not None:
            batch_size = preds.shape[0]
            if batch_size > 0:
                # 라벨을 One-Hot 벡터로 변환하여 거리 계산
                labels_one_hot = F.one_hot(labels, num_classes=self.num_classes).float()
                sdtw_sum = 0.0
                for i in range(batch_size):
                    p = preds[i].unsqueeze(0)
                    l = labels_one_hot[i].unsqueeze(0)
                    sdtw_sum += self.sdtw(p, l)
                
                loss += 0.1 * (sdtw_sum / batch_size)
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if not cfg.MIXUP.ENABLED:                                                            
           ...
           mixup_fn = Mixup(                                                                 
               mixup_alpha=cfg.MIXUP.ALPHA, cutmix_alpha=cfg.MIXUP.CUTMIX_ALPHA, cutmix_minma
x=cfg.MIXUP.CUTMIX_MINMAX, prob=cfg.MIXUP.PROB, switch_prob=cfg.MIXUP.SWITCH_PROB, mode=cfg.M
IXUP.MODE,
